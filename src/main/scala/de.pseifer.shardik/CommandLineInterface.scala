...
class CommandLineInterface(baseConfiguration: Configuration, arguments: Seq[String])
  ...
  footer(
    "\n" +
      ...
      |    https://github.com/pseifer/shardik
  ...
  def toConfiguration: SharTry[Configuration] =
    for
      i <- loadOntology
      s <- loadSources
      p <- loadPrefixes
